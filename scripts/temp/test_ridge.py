...
def getFeatureCache(features, instances):
	cache = {}
	i = -1
	for inst in instances:
		prefix = '\t'.join(inst)
		cands = inst[len(inst)-2:]
		for cand in cands:
			i += 1
			cache[prefix+'\t'+cand] = features[i]
	return cache
...
class OnlineRegressionRanker:
	...
	def trainRegressionModel(self, training_data_text):
		...
		for line in training_data_text.strip().split('\n'):
			...
			featmap = {}
			featprefix = line.strip()+'\t'
			for cand, index in zip(cands, indexes):
				c += 1
				featmap[cand] = self.fc[featprefix+str(index)+':'+cand]
...
features = fe.calculateFeatures(datapath, format='victor')
featcache = getFeatureCache(features, instances)
...
for i in range(0, ntrains):
	...
	for j in range(1, step)+range(step, pivot+1, step):
		...
		ranker = OnlineRegressionRanker(featcache)
...
#Save results:
o = open('../corpora/temp/ridgebin_'+name+'_'+str(trainprop)+'_'+str(ntrains)+'_'+str(step)+'.txt', 'w')
