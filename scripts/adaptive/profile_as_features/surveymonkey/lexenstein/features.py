...
class FeatureEstimator:
	...
	def oneHotFeature(self, data, args):
                result = args[0]
                return result
...
##############################################################################################################################################################################################################################################
	...
	def addOneHotFeatures(self, name, values, orientation):
		if orientation not in ['Complexity', 'Simplicity']:
                        print('Orientation must be Complexity or Simplicity')
                else:
                        if 'OneHot:'+name not in self.resources:
				enc = OneHotEncoder()
				X = enc.fit_transform(values)
				self.resources['OneHot:'+name] = X
                        self.features.append((self.oneHotFeature, [self.resources['OneHot:'+name]]))
                        self.identifiers.append(('One Hot Feature: '+name, orientation))
